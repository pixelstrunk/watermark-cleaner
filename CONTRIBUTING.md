...
2. If code changes are needed, matching changes in `python/wmc/` and `node/src/`.
...
```
git clone https://github.com/pixelstrunk/wmc-cleaner
cd wmc-cleaner
...
- Local rewrite backend: drive `wmc rewrite` through a local model (for example Ollama) as a zero-network alternative to DeepL.
